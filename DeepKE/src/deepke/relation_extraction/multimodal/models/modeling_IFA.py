...
class IFAModel(nn.Module):
    def __init__(self, vision_config, text_config, config,add_pooling_layer=True):
        ...
        # hzk
        self.bertmodel=BertModel.from_pretrained("/home/huang/代码/DeepKE/src/model/bert-base-uncased",config=text_config)

        # HZK
        self.HZK=HZK(config)
        self.HZK2=HZK2(config)
    ...
    def forward(
        ...
        # 32,768
        bert_output = torch.sum(masked_output, dim=1, keepdim=False) / mask_len
        # bert_output = torch.sum(masked_output, dim=1, keepdim=True) / mask_len
        ...
        a,b,c,T_recon,V_recon = self.HZK2(vision_embedding_output=sequence_output, text_embedding_output=sequence_output)
        ...
        ),a,b,c,BaseModelOutputWithPooling(
            last_hidden_state=hzk_v_outputs,
