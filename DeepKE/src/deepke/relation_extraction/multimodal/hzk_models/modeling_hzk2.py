...
class HZK2(nn.Module):
    ...
    def forward(self, vision_embedding_output, text_embedding_output, lengths=None,
        ...
        a = contrastiveLossAdapter().to('cuda')
        get_contrastive_loss = a(self.utt_shared_t, self.utt_private_t)
        get_contrastive_loss+= a(self.utt_shared_v, self.utt_private_v)
        get_contrastive_loss+= a(self.utt_private_t, self.utt_private_v)
        # get_contrastive_loss=self.get_contrastive_loss()
        # 压缩
        ...
        return self.config.diff_weight * get_contrastive_loss , self.config.cmd_weight * self.get_cmd_loss(),self.config.recon_weight*self.get_recon_loss(),T_recon,V_recon
    ...
    def get_contrastive_loss(self ):
        a=contrastiveLossAdapter()
        loss=a(self.utt_shared_t,self.utt_private_t)
        loss+=a(self.utt_shared_v,self.utt_private_v)
        loss+=a(self.utt_private_t,self.utt_private_v)
        return  loss
...
class contrastiveLossAdapter(nn.Module):
    ...
    def forward(self,inputT,inputV):
        ...
        emb_V=self.gelu(self.fc1((inputV.transpose(1,2)).to('cuda')).to('cuda').squeeze(2))
        emb_T=self.gelu(self.fc2((inputT.transpose(1,2)).to('cuda')).to('cuda').squeeze(2))
