...
from deepke.relation_extraction.multimodal.models.IFA_model import IFAREModel,IFAREModel_base,IFAREModel_cat,IFAREModel_recon
...
from deepke.relation_extraction.multimodal.modules.data_set import MyDataset
...
def main(cfg):
    ...
    import itertools
    a = [5.5e-3,5.7e-3,5.9e-3,6e-3,6.1e-3,6.3e-3,6.5e-3]
    b = [1,8e-1,6e-1,5e-1,4e-1,2e-1,1e-1]
    combinations = list(itertools.product(a, b))
    for lr, weight_decay in combinations:
        ...
        # print("cat")
        # model = IFAREModel_cat(num_labels, tokenizer, cfg)
        # trainer = Trainer(train_data=train_dataloader, dev_data=dev_dataloader, test_data=test_dataloader, re_dict=re_dict,
        #                   model=model, args=cfg, logger=logger, writer=writer)
        # trainer.train()
        #
        #
        # print("base")
        # model = IFAREModel_base(num_labels, tokenizer, cfg)
        ...
        trainer = Trainer(train_data=train_dataloader, dev_data=dev_dataloader, test_data=test_dataloader, re_dict=re_dict,model=model, args=cfg, logger=logger, writer=writer, lr=lr, weight_decay=weight_decay)
        trainer.train()
